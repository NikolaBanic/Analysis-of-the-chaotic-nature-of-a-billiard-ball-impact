...
def pool(fi_start, fi_stop, n, speed, X_P, Y_P, plot = False, animate = False, analiza = False):
    ...
    if plot:
        ...
        plt.title('Analiza udarca biljarske kugle')
        plt.axis('equal')
        plt.ylabel('Visina stola za biljar [m]')
        plt.xlabel('Širina stola za biljar [m]')
    ...
    for i in np.arange(fi_start, fi_stop, n):
        ...
        while abs(tan_v0) >= 0.1:
            ...
            if pogodak == 1:
                print(f'Pogodak u rupu broj {H}')
                res.append(f'Pogodak u rupu broj {H}')
                break
    ...
    if plot:
        if animate:
            ...
            kut_template = 'Kut = %.f stupnjeva'
            budarac_template = 'Broj udaraca = %.f'
            bpromasaj_template = 'Broj promašaja = %.f'
            bpogotka_template = 'Broj pogotka = %.f'
            ...
            def animate(i):
                ...
                kut_text.set_text(kut_template%(angle[i]))      
                res_text.set_text(res[i])

                hole1_text.set_text(budarac_template%(len(S)))
                hole2_text.set_text(bpromasaj_template%(count_0))
                hole3_text.set_text(bpogotka_template%(count_hit))
            ...
            # anim.save(f'{res_dir}/test_anim.avi', fps = 10, dpi = 300)
        plt.show()
    ...
    if analiza:
        ...
        axes[0].set_xlabel('Kut udarca')
        axes[0].set_ylabel('Oznaka rupe: 1- 6, Promašaj: 0')
        axes[0].set_title(f'Analiza udarca prema rasponu kuteva između: {fi_start}° i {fi_stop}°')
        axes[0].text(0.70, 0.90, f'Početna brzina udarca: {speed} m/s', transform=axes[0].transAxes)
        axes[0].text(0.70, 0.95, f'Razmak kuteva između udarca: {n}°', transform=axes[0].transAxes)
        lbl1 = axes[1].bar(0, count_0, label = 'Broj promašaja', color = 'red')
        lbl2 = axes[1].bar(1, count_hit, label = 'Broj pogotka', color = 'green')
        lbl3 = axes[1].bar([2, 3, 4, 5, 6, 7], [count_1, count_2, count_3, count_4, count_5, count_6],
                    label = 'Pogodak u rupu broj', color = 'blue')
        axes[1].legend(loc = 'upper right')
        axes[1].set_ylabel('Broj pogotka/promašaja u %')
        axes[1].set_title('Broj pogotka/promašaja po oznaci rupe')
        labels_hole = ['Promašaj', 'Pogodak', 'Rupa 1', 'Rupa 2', 'Rupa 3', 'Rupa 4', 'Rupa 5', 'Rupa 6']
        ...
        fig.suptitle('Analiza kaotičnosti putanje biljarske kugle')
        
        plt.savefig(f'{res_dir}/{fi_start}_{fi_stop}_{n}_{speed}_udarac_analiza.png', dpi = 300)
        plt.show()
...
if __name__ == '__main__': 
    ...
    delta = [0.5, 1, 2, 3, 4]
    labels = ['Promašaj', 'Pogodak', 'Rupa 1', 'Rupa 2', 'Rupa 3', 'Rupa 4', 'Rupa 5', 'Rupa 6']
    ...
    for i in range(len(delta)):
        ...
        for j in range(8):
            R_i.append(R[5][0, j])
        bar = axes.bar(x_width + w[i], R_i, width, color = colors[i], label = f'Δ kut = {delta[i]}°', alpha = 0.9)
        if i % 2 == 1:
            axes.bar_label(bar, fmt = '%.1f%%', padding = 4)
        else:
            axes.bar_label(bar, fmt = '%.1f%%', label_type = 'center')
    axes.legend(loc = 'upper right')
    axes.text(0.77, 0.92, f'Početna brzina udarca: {speed} m/s', transform=axes.transAxes)
    axes.set_xticks(x_width, labels)
    axes.set_ylabel('Broj pogotka/promašaja u %')
    axes.set_title('Analiza udarca biljarske kugle prema Δ kutu')
    plt.savefig('slike_i_animacija/analiza_udarca_solo_plot.png', dpi = 300)
    plt.show()
    ...
    cbar = fig.colorbar(contourf_, label = 'Oznaka rupe upada biljarske kugle 1- 6; 0 - promašaj')
    axes.set_xticks(X_, x_scale)
    plt.xlabel('Pozicija x-osi početnog udarca prema [%] širini stola za billjar, (0.991 m)')
    plt.ylabel('Kut udarca [°]')
    plt.title('2D analiza upada biljarske kugle u rupu ovisno o početnoj poziciji kugle')
    axes.text(0.02, 0.02, f'Početna brzina udarca: {speed} m/s, delta_kut: {delta}°', transform=axes.transAxes)
    plt.savefig('slike_i_animacija/2D_analiza.png', dpi = 300)
    plt.show()
    ...
    for i in range(len(delta)):
        ...
        axes[i].set_yticks([])
        axes[i].set_ylabel(f'Δ kut = {delta[i]}°')
        axes[i].text(0.01, 0.85, f'Speed = {speed} m/s, Δ kut = {delta[i]}°', transform=axes[i].transAxes)
    plt.suptitle('Analiza udarca biljarske kugle prema Δ kutu')
    plt.xlabel('Kut udarca [°]')
    cbar = fig.colorbar(contourf_, ax=axes.ravel().tolist(), label = 'Oznaka rupe upada kugle: 1 - 6, Promašaj: 0')
    plt.savefig('slike_i_animacija/subplots_1.png', dpi = 300)
    plt.show()